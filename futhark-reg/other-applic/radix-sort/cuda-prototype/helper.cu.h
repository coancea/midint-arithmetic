...
void printMatrix(uint* matrix, int num_blocks, int num_elems_in_block) {
      for(int b=0; b<num_blocks; b++) {
        printf("\t");
        for(int q=0; q<num_elems_in_block; q++) {
            printf("%d, ", (uint32_t)matrix[b*num_elems_in_block+q]);
        }
        printf("\n");
    }
}
