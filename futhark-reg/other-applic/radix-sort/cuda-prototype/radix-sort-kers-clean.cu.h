...
void ker2( size_t N, int bit_beg
    ...
    {
        size_t glb_offs = blockIdx.x*histo_len;
        histo_loc[threadIdx.x] = d_histo[glb_offs + threadIdx.x];
        __syncthreads();
        int loc_el = scanIncBlock< Add<uint16_t> >(histo_loc, threadIdx.x);        
        __syncthreads();
        uint64_t glb_el = d_histoST[glb_offs + threadIdx.x];
        histo_glb[threadIdx.x] = glb_el - loc_el; 
    }
    __syncthreads();
