...
def mkShinvInput (num_instances: i64) (m: i64) (q2: i64)
...
--  let rng_state  = rng_master
  ...
  let (_, vss) = unzip <| opaque <| map (mkRandBIntPart max_prec m q2) rngs
...
entry mkShinvInpRep  (num_instances: i64) (m: i64) (q2: i64)
                   : ( [num_instances][m][q2]u64
                     , [num_instances]i32
                     ) =
  let (vss, hss) = mkShinvInput 1i64 m q2
  in (replicate num_instances vss[0], replicate num_instances hss[0])
...
---- ==
-- entry: shinv2048Q8
-- "Shinv2048Q8" script input { mkShinvInpRep 32768 256 8 }
